...
void main(){
  ...
  // Option 2 water tint: the fallback tint follows the pool's long axis,
  // rather than always using vSimUV.y. This prevents the flat water plane
  // from reading as a left-to-right screen gradient when the pool length is
  // actually mapped along the X axis.
  float tUV = poolSize.x >= poolSize.y ? clamp(vSimUV.x, 0.0, 1.0) : clamp(vSimUV.y, 0.0, 1.0);

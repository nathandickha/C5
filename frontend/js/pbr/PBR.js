...
export class PBRManager {
  ...
  buildTileMaterial(maps, mesh, surfaceKind = "default") {
    ...
    mat.onBeforeCompile = (shader) => {
      shader.uniforms.uPoolDepthTintShallow = { value: new THREE.Color(0.90, 0.985, 1.0) };
      shader.uniforms.uPoolDepthTintDeep = { value: new THREE.Color(0.18, 0.68, 0.92) };
      shader.uniforms.uPoolDepthTintStrength = { value: 0.68 };
      shader.uniforms.uPoolDepthTintBrighten = { value: 0.08 };
...
float poolDepthTintAmount = mix(0.16, uPoolDepthTintStrength, poolDepthTintFactor);
vec3 tintedPoolColour = mix(diffuseColor.rgb, poolDepthTintColor, poolDepthTintAmount * 0.42);
tintedPoolColour *= mix(vec3(1.0), poolDepthTintColor, poolDepthTintAmount * 0.58);
tintedPoolColour += uPoolDepthTintBrighten * (1.0 - poolDepthTintFactor) * vec3(0.60, 0.95, 1.0);

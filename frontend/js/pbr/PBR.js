...
export class PBRManager {
  ...
  buildTileMaterial(maps, mesh, surfaceKind = "default") {
    ...
    // Option 1 water tint: keep the water plane mostly clear, then tint the
    // underwater tile material by actual world Z. This avoids a left-to-right
    // gradient on the flat water surface and makes lower/deeper surfaces read
    // as slightly deeper blue.
    mat.onBeforeCompile = (shader) => {
      shader.uniforms.uPoolDepthTintShallow = { value: new THREE.Color(0.92, 0.985, 1.0) };
      shader.uniforms.uPoolDepthTintDeep = { value: new THREE.Color(0.42, 0.78, 0.92) };
      shader.uniforms.uPoolDepthTintStrength = { value: 0.36 };
      shader.vertexShader = `varying float vPoolTileWorldZ;\n` + shader.vertexShader.replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\nvec4 poolTileWorldPosition = modelMatrix * vec4(transformed, 1.0);\nvPoolTileWorldZ = poolTileWorldPosition.z;'
      );
      shader.fragmentShader = `uniform vec3 uPoolDepthTintShallow;\nuniform vec3 uPoolDepthTintDeep;\nuniform float uPoolDepthTintStrength;\nvarying float vPoolTileWorldZ;\n` + shader.fragmentShader.replace(
        '#include <map_fragment>',
        `#include <map_fragment>
float poolDepthTintFactor = smoothstep(-0.20, -2.35, vPoolTileWorldZ);
vec3 poolDepthTintColor = mix(uPoolDepthTintShallow, uPoolDepthTintDeep, poolDepthTintFactor);
float poolDepthTintAmount = mix(0.08, uPoolDepthTintStrength, poolDepthTintFactor);
diffuseColor.rgb = mix(diffuseColor.rgb, diffuseColor.rgb * poolDepthTintColor, poolDepthTintAmount);`
      );
    };

...
export class PBRManager {
  ...
  buildTileMaterial(maps, mesh, surfaceKind = "default") {
    ...
    // Option 1 v2 water tint: keep the water plane mostly clear, then tint
    // the underwater tile material using actual world height. This version is
    // deliberately stronger and checks both Y-up and Z-up coordinates so it
    // still works if the pool geometry uses either vertical axis.
    mat.onBeforeCompile = (shader) => {
      shader.uniforms.uPoolDepthTintShallow = { value: new THREE.Color(0.00, 1.0, 1.0) };
      shader.uniforms.uPoolDepthTintDeep = { value: new THREE.Color(0.00, 0.05, 1.0) };
      shader.uniforms.uPoolDepthTintStrength = { value: 1.85 };
      shader.uniforms.uPoolDepthTintBrighten = { value: 0.22 };
      shader.vertexShader = `varying vec3 vPoolTileWorldPos;
` + shader.vertexShader.replace(
        '#include <begin_vertex>',
        '#include <begin_vertex>\nvec4 poolTileWorldPosition = modelMatrix * vec4(transformed, 1.0);\nvPoolTileWorldPos = poolTileWorldPosition.xyz;'
      );
      shader.fragmentShader = `uniform vec3 uPoolDepthTintShallow;\nuniform vec3 uPoolDepthTintDeep;\nuniform float uPoolDepthTintStrength;\nuniform float uPoolDepthTintBrighten;\nvarying vec3 vPoolTileWorldPos;\n` + shader.fragmentShader.replace(
        '#include <map_fragment>',
        `#include <map_fragment>
// Some pool meshes are Z-up while others can be Y-up depending on geometry path.
// Use whichever vertical-depth reading is stronger, so the tint cannot silently disappear.
float poolDepthFromZ = smoothstep(-0.10, -2.40, vPoolTileWorldPos.z);
float poolDepthFromY = smoothstep(-0.10, -2.40, vPoolTileWorldPos.y);
float poolDepthTintFactor = max(poolDepthFromZ, poolDepthFromY);
vec3 poolDepthTintColor = mix(uPoolDepthTintShallow, uPoolDepthTintDeep, poolDepthTintFactor);
float poolDepthTintAmount = mix(0.75, uPoolDepthTintStrength, poolDepthTintFactor);
vec3 tintedPoolColour = mix(diffuseColor.rgb, poolDepthTintColor, clamp(poolDepthTintAmount * 0.86, 0.0, 1.0));
tintedPoolColour *= mix(vec3(1.0), poolDepthTintColor, clamp(poolDepthTintAmount * 0.92, 0.0, 1.0));
tintedPoolColour += uPoolDepthTintBrighten * (1.0 - poolDepthTintFactor) * vec3(0.0, 1.0, 1.0);
diffuseColor.rgb = clamp(tintedPoolColour, 0.0, 1.0);`
    ...
    mat.userData.isOption1DepthTintOverlayExtremeTest = true;
